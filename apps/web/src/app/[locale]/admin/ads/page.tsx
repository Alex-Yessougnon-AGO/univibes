    ...
    >
      <motion.div variants={fadeUp} className="flex items-center justify-between mb-6">
        <div>
          ...
          <h1 className="text-2xl font-[family-name:var(--font-display)] text-[var(--text)] tracking-tight mb-1">{t("admin.ads")}</h1>
          <p className="text-sm text-[var(--text-secondary)]">{t("admin.adsManagementDesc")}</p>
        </div>
          ...
          <div key={ad.id} className="rounded-2xl bg-[var(--surface)] border border-[var(--border)] overflow-hidden shadow-[var(--shadow-sm)]">
            ...
            <div className="p-4 space-y-2.5">
              <div className="flex items-center justify-between">
                ...
                <Badge variant={ad.active ? "success" : "soft"}>{ad.active ? t("admin.active") : t("admin.inactive")}</Badge>
              </div>
              <div className="grid grid-cols-2 gap-2 text-[11px]">
                <div><span className="text-[var(--text-tertiary)]">{t("admin.city")} :</span><span className="text-[var(--text-secondary)] ml-1">{ad.city}</span></div>
                <div><span className="text-[var(--text-tertiary)]">{t("admin.impressions")} :</span><span className="text-[var(--text-secondary)] ml-1">{ad.impressions}</span></div>
                <div><span className="text-[var(--text-tertiary)]">{t("admin.dateStart")} :</span><span className="text-[var(--text-secondary)] ml-1">{ad.start}</span></div>
                <div><span className="text-[var(--text-tertiary)]">{t("admin.clicks")} :</span><span className="text-[var(--text-secondary)] ml-1">{ad.clicks}</span></div>
              </div>
              <div className="pt-2 border-t border-[var(--border-subtle)]">
                <div className="flex items-center justify-between text-xs">
                  <span className="text-[var(--text-tertiary)]">{t("admin.ctr")}</span>
