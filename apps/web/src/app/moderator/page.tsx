    ...
    <div className="min-h-dvh bg-[var(--bg)]">
      <header className="sticky top-0 z-40 glass border-b border-[var(--border)]">
        <div className="flex items-center justify-between h-14 px-4 max-w-5xl mx-auto">
          ...
          <Badge variant="warning" className="text-[10px]">{PENDING.length} en attente</Badge>
        </div>
      </header>
            ...
            {PENDING.map((item) => (
              <div key={item.name} className="flex items-center justify-between p-4 hover:bg-[var(--border-subtle)] transition-colors border-b border-[var(--border)] last:border-b-0">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button variant="ghost" size="sm" asChild><Link href={`/moderator/events/${item.name.toLowerCase().replace(/\s+/g, "-")}/review`}>Détails</Link></Button>
                </div>
              </div>
            ))}
          </motion.div>
